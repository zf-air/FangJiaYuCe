...
def query(ccity,chengqu,chengqu_h):
    print("正在从数据库中查询，请稍等：")
    db = pymysql.connect(host="localhost", user="root", password="root", database="homeless", charset="utf8mb4")
    cursor = db.cursor()
    ...
    db = pymysql.connect(host="localhost", user="root", password="root", database="homeless", charset="utf8mb4")
    cursor = db.cursor()
    cursor.execute("SELECT * FROM color WHERE city_pname = '%s'" % ccity)
    r = cursor.fetchall()
...
def updatecity(cityname,shijunjia):# 更新市均价
    db = pymysql.connect(host="localhost", user="root", password="root", database="homeless", charset="utf8mb4")
    cursor = db.cursor()
    cursor.execute("UPDATE color SET shijunjia = '%s' WHERE city_pname='%s'" % (shijunjia, cityname))
    db.commit()
    db.close()
